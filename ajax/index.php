...
function LoadingAttendance()
{
	...
	if (!empty($requestData['search']['value'])) {
		...
		$sql .= " AND (`att`.`attendance_date` LIKE '%" . $searchValue . "%'";
		$sql .= " OR CONCAT(TRIM(`emp`.`first_name`), ' ', TRIM(`emp`.`last_name`)) LIKE '%" . $searchValue . "%'";
		$sql .= " OR CONCAT(`att`.`action_time`) LIKE '%" . $searchValue . "%'";
		$sql .= " OR CONCAT(`att`.`emp_desc`) LIKE '%" . $searchValue . "%')";
	}
    ...
    if ($encoded_data !== false) {
        ...
        $error_response = array(
            "error" => "Failed to encode response data to JSON"
        );
